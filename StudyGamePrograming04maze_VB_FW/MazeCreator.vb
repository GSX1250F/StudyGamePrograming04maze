...
Public Module MazeCreator
    ...
    Public Function MazeCreate(ByRef width As Integer, ByRef height As Integer) As List(Of List(Of Integer))
        ...
        If (width >= 5 And height >= 5) Then
            ...
            Dim b(50) As Byte
            rng.GetBytes(b)
            startCell.X = 2 * Int(b(0) / 256 * 3) + 1
            startCell.Y = 2 * Int(b(1) / 256 * 3) + 1
            mCell = startCell
            rng.Dispose()
            '穴掘り開始
            digCount = 0
    ...
    Private Sub Dig(ByVal Cell As Vector2)
        ...
        While (True)
            ...
            Else
                ...
                Dim b(0) As Byte
                rng.GetBytes(b)
                Dim dirIndex As Integer = Int(b(0) / 256 * directions.Count)
                rng.Dispose()
                '決まった方向に先2マス分を通路とする
    ...
    Private Function GetStartCell() As Boolean
        ...
        Else
            ...
            Dim b(0) As Byte
            rng.GetBytes(b)
            Dim Index As Integer = Int(b(0) / 256 * startCells.Count())
