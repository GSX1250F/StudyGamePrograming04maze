Public Class SpriteComponent
    ...
    Public Overridable Sub Draw(ByRef mGraphics As Object)
        If (mTexture IsNot Nothing) And (GetVisible() = True) Then
            ...
            Dim x1 As Integer = Int((-w / 2) * Math.Cos(angle) + (-h / 2) * Math.Sin(angle) + x0)
            Dim y1 As Integer = Int(-(-w / 2) * Math.Sin(angle) + (-h / 2) * Math.Cos(angle) + y0)
            Dim x2 As Integer = Int(w / 2 * Math.Cos(angle) + (-h / 2) * Math.Sin(angle) + x0)
            Dim y2 As Integer = Int(-w / 2 * Math.Sin(angle) + (-h / 2) * Math.Cos(angle) + y0)
            Dim x3 As Integer = Int((-w / 2) * Math.Cos(angle) + h / 2 * Math.Sin(angle) + x0)
            Dim y3 As Integer = Int(-(-w / 2) * Math.Sin(angle) + h / 2 * Math.Cos(angle) + y0)
            'PointF配列を作成
